...
function handle_upload(string $inputName, string $targetDir, string $publicPrefix): ?string
{
    ...
    if (!upload_move($file['tmp_name'], $targetPath)) {
...
/**
 * Verschiebt eine hochgeladene Datei an den Zielort und verschlüsselt sie dort (AES-256-GCM, siehe crypto.php).
 * Dateien aus der verschlüsselten API-Übertragung sind keine PHP-Uploads und werden per rename() übernommen.
 * Solange noch kein Schlüssel eingerichtet ist (tools/setup-encryption.php), bleibt die Datei unverschlüsselt.
 */
function upload_move(string $tmpName, string $targetPath): bool
{
    $viaApi = isset($GLOBALS['api_envelope_files'][$tmpName]);
    $moved = $viaApi ? @rename($tmpName, $targetPath) : move_uploaded_file($tmpName, $targetPath);
    if (!$moved) {
        return false;
    }
    @chmod($targetPath, 0640);
    if (is_readable(crypto_key_file())) {
        crypto_file_encrypt($targetPath);
    }
    return true;
}
